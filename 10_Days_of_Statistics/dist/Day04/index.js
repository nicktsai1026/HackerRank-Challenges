...
function calculateBirthProbability() {
    let probability = 0;
    for (var numBoys = 3; numBoys <= 6; numBoys++) {
        let numGirls = 6 - numBoys;
        // Calculate the probability of this number of boys and girls using the binomial distribution
        const binomialCoeff = factorial(6) / (factorial(numBoys) * factorial(numGirls));
        const binomialProb = binomialCoeff * Math.pow(boyGirlRatio, numBoys) * Math.pow(1 - boyGirlRatio, numGirls);
        probability += binomialProb;
    }
    return probability;
}
const birthProbability = calculateBirthProbability().toFixed(3);
...
function calculateRejectRatio(start, end) {
    let probability = 0;
    for (var i = start; i <= end; i++) {
        const binomialProb = binomialCoefficient(pistons, i) * Math.pow(rejectedRatio, i) * Math.pow(1 - rejectedRatio, pistons - i);
        probability += binomialProb;
    }
    return probability;
}
let probabilityA = calculateRejectRatio(0, 2);
let probabilityB = calculateRejectRatio(2, pistons);
...
*/
const defectiveRatio = 1 / 3;
const target = 5;
function calculateDefectiveProduct(num) {
